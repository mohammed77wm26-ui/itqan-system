...
# --- 1. بيانات الطلاب (إضافة، بحث، وتعديل) ---
if menu == "🏠 بيانات الطلاب":
    ...
    st.subheader("🔍 ابحث عن طالب لتعديله أو أضف جديداً")
    student_list = ["➕ إضافة طالب جديد"] + BIO['الاسم'].tolist()
    selected_name = st.selectbox("اختر الاسم من هنا للتحكم في بياناته", student_list)

    # تعبئة البيانات تلقائياً عند الاختيار
    if selected_name != "➕ إضافة طالب جديد":
        s_data = BIO[BIO['الاسم'] == selected_name].iloc[0]
        v_name, v_age, v_grade, v_id, v_phone, v_email = s_data['الاسم'], str(s_data['العمر']), s_data['الصف'], str(s_data['الرقم']), str(s_data['الهاتف']), s_data['الإيميل']
        btn_label = "تحديث البيانات الحالية"
    else:
        v_name, v_age, v_grade, v_id, v_phone, v_email = "", "", "", "", "", ""
        btn_label = "حفظ كطالب جديد"

    with st.form("edit_form", clear_on_submit=(selected_name == "➕ إضافة طالب جديد")):
        name = st.text_input("الاسم الثلاثي", value=v_name)
        col1, col2 = st.columns(2)
        with col1:
            age = st.selectbox("العمر", [""] + [str(i) for i in range(5, 50)], index=([str(i) for i in range(5, 50)].index(v_age)+1 if v_age else 0))
        ...
        if st.form_submit_button(btn_label):
            if name and s_id:
                # حذف القديم إذا كان تعديلاً لإضافة الجديد مكانه
                updated_bio = BIO[BIO['الاسم'] != selected_name] if selected_name != "➕ إضافة طالب جديد" else BIO
                new_row = pd.DataFrame([[s_id, name, age, grade, phone, email]], columns=BIO.columns)
                pd.concat([updated_bio, new_row], ignore_index=True).to_csv('db_bio.csv', index=False)
